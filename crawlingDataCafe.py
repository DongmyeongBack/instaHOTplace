...
if os.path.isfile('crawlingdataCafe.csv'):
    print("There is original file")
    f=open('crawlingdataCafe.csv', 'a', newline='', encoding='utf-8-sig')
    wr = csv.writer(f)
else:
    print("Make new file")
    f=open('crawlingdataCafe.csv', 'w', newline='',encoding='utf-8-sig')
    wr = csv.writer(f)
...
command = "git add crawlingdataCafe.csv"
...
command = f"git commit -m '{date}cafe'"
